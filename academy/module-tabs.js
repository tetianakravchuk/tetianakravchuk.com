(() => {
  ...
  const symbols = { learn: '▱', evidence: '◉', code: '</>', quiz: '?', interview: '◯', mastery: '✦' };
  ...
  const setStepComplete = (step, complete) => {
    if (!activeModuleId) return;
    const state = read();
    const completed = new Set(state[activeModuleId] || []);
    complete ? completed.add(step) : completed.delete(step);
    state[activeModuleId] = [...completed];
    write(state);
  };
  ...
  layout.className = 'academy-clean-layout';
  const main = document.createElement('div');
  main.className = 'academy-clean-main';
  const sidebar = document.createElement('aside');
  sidebar.className = 'academy-clean-sidebar';
  ...
  progressCard.className = 'academy-clean-card academy-clean-progress';
  ...
  infoCard.className = 'academy-clean-card academy-clean-info';
  infoCard.innerHTML = `<h3>About this module</h3><dl><div><dt>Estimated time</dt><dd>15–20 min</dd></div><div><dt>Difficulty</dt><dd><span>Beginner</span></dd></div><div><dt>Focus</dt><dd>Product & Strategy</dd></div><div><dt>Prerequisite</dt><dd>None</dd></div></dl><div class="academy-clean-tip"><strong>Learning tip</strong><p>Move in order: Learn → Evidence → Code → Quiz → Interview. Mark each step complete only after you actually do it.</p></div>`;
  sidebar.append(progressCard, infoCard);
  layout.append(main, sidebar);
  content?.appendChild(layout);

  const stepAction = document.createElement('div');
  stepAction.className = 'academy-clean-step-action';
  stepAction.innerHTML = `<button type="button" data-step-complete>Mark this step complete</button>`;
  main.appendChild(stepAction);

  const footer = document.createElement('div');
  footer.className = 'academy-clean-footer';
  footer.innerHTML = `<button type="button" class="academy-clean-back" data-module-back>← Previous</button><button type="button" class="academy-clean-module-complete" data-module-complete>Mark module complete</button><button type="button" class="academy-clean-next" data-module-next>Continue →</button>`;
  content?.appendChild(footer);

  function renderProgress() {
    const completed = completedForModule();
    const pct = Math.round((completed.size / steps.length) * 100);
    const ring = progressCard.querySelector('.academy-clean-ring');
    ring.style.setProperty('--pct', `${pct * 3.6}deg`);
    ring.querySelector('strong').textContent = `${pct}%`;
    ring.querySelector('span').textContent = pct === 100 ? 'Complete' : pct ? 'In progress' : 'Not started';
    const list = progressCard.querySelector('.academy-clean-progress-list');
    list.replaceChildren();
    steps.forEach(step => {
      const row = document.createElement('div');
      row.className = 'academy-clean-progress-row';
      row.innerHTML = `<span class="academy-clean-dot ${completed.has(step) ? 'is-done' : ''}" aria-hidden="true">${completed.has(step) ? '✓' : ''}</span><span>${labels[step]}</span>`;
      list.appendChild(row);
    });
    tabs.forEach(tab => tab.classList.toggle('is-complete', completed.has(tab.dataset.moduleTab)));
    const action = stepAction.querySelector('[data-step-complete]');
    action.textContent = completed.has(activeStep) ? '✓ Step complete' : 'Mark this step complete';
    action.classList.toggle('is-complete', completed.has(activeStep));
  }
  ...
  function showStep(step, { focus = false } = {}) {
    ...
    tabs.forEach(tab => {
      const selected = tab.dataset.moduleTab === step;
    ...
    stepAction.hidden = step === 'mastery';
    renderProgress();
  ...
  stepAction.querySelector('[data-step-complete]').addEventListener('click', () => {
    const completed = completedForModule();
    setStepComplete(activeStep, !completed.has(activeStep));
    renderProgress();
  });
  ...
  footer.querySelector('[data-module-complete]').addEventListener('click', () => {
    const completed = completedForModule();
    if (steps.slice(0, 5).some(step => !completed.has(step))) {
      footer.querySelector('[data-module-complete]').textContent = 'Complete the 5 study steps first';
      setTimeout(() => footer.querySelector('[data-module-complete]').textContent = 'Mark module complete', 1800);
      return;
    }
    setStepComplete('mastery', true);
    dialog.querySelector('[data-set-status="mastered"]')?.click();
    renderProgress();
  ...
  const observer = new MutationObserver(() => {
    ...
    if (moduleId && moduleId !== activeModuleId) {
      ...
      activeStep = 'learn';
      footer.querySelector('[data-module-complete]').textContent = 'Mark module complete';
      showStep('learn');
    }
  });
  observer.observe(dialog, { attributes: true, attributeFilter: ['open'] });
  dialog.addEventListener('close', () => { activeModuleId = null; activeStep = 'learn'; });
  if (dialog.open) { activeModuleId = moduleIdFromDialog(); showStep('learn'); }
  else showStep('learn');
})();